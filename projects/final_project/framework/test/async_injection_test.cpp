...
void TestSyncScheduler()
{
    ...
    sched->AddTask(task_ptr, 2000);
    sched->AddTask(task_ptr, 4000);
    sched->AddTask(task_ptr, 6000);
    sleep(7);
}
...
void TestReentrantTask()
{
    ...
    sched->AddTask(task, 1000);
    sleep(4);
}

bool AsyncInjectionFunc()
{
    auto now = std::chrono::system_clock::now();
    std::time_t time_now = std::chrono::system_clock::to_time_t(now);
...
{
    std::cout << BLUE << "TestAsyncInjection" << std::endl;
    std::cout << "Should print 1-5 every 1 second" << DEFAULT << std::endl;

    AsyncInjection* ptr = new AsyncInjection(AsyncInjectionFunc, 1000);
    sleep(8);
}
